...
public class RemovedDuplicatesSortedArray {
    ...
    /*
    TC : O(n)
    SC : O(n) // worstcase , say there are no duplicates.

    Using hashset solution works for unsorted array as well.
     */
    public static int removeDuplicatesUsingExtraSpace(int[] nums) {

        int n = nums.length;
        if(n == 1)return n; // no duplicates

        HashSet<Integer> hset = new HashSet<Integer>();
        int ri = 0; // pointer to keep track of result end window
        hset.add(nums[0]);
    ...
    public static int removeDuplicatesSortedEfficient(int[] nums) {
